...
newtype Unevaluated a = Unevaluated { unwrapUnevaluated :: a }
...
newtype DeclaredSymbols = DeclaredSymbols
    { unwrapDeclaredSymbols :: HS.HashSet Binding
    }
...
isDefined :: (MonadReader r m, Field1' r DeclaredSymbols, ToBinding b)
          => b -> m Bool
isDefined b = asks (HS.member (toBinding b) . unwrapDeclaredSymbols . (^. _1))


isSf :: (MonadReader r m, Field2' r Registry, ToBinding b)
     => b -> m (Maybe QualifiedBinding)
isSf b' = (($ b) . registryResolveSf) <$> view _2
  where b = toBinding b'


isAlgo :: (MonadReader r m, Field2' r Registry, ToBinding b)
       => b -> m (Maybe Algo)
...
assignIds e = do
    ...
    -- reset the id counter to the highest id currently assigned so as
    -- to avoid clashes futher in the compiler
  ...
  where
    ids = cata f e
    f (VarF (Sf _ (Just id))) = HS.singleton id
    f e = F.fold e
    genNewId = do
        id <- generateId
        if id `HS.member` ids
...
getCljSfnInitExpr :: Annotated Meta a -> Maybe Object
getCljSfnInitExpr st =
    st ^? annotation . to getMeta . _Just >>= flip Clojure.get initExprKw
  where
    getMeta (Meta m) = m
...
expectUnqual (Symbol Nothing name) = make name
expectUnqual sym =
    failWith $ "Expected unqualified symbol, got " <> Str.showS sym


expectSym :: (MonadError Error m, Show a) => Str.Str -> ExprF a -> m Binding
expectSym _ (SymF s) = pure $ symToBinding s
expectSym location thing =
    failWith $
    "Expected symbol in " <> location <> ", found " <> Str.showS thing


mkLams ::
       (Applicative m, MonadGenBnd m)
    => [Assignment]
    -> m (AL.Expr a -> AL.Expr a)
...
handleAssign =
    project >>> \case
        SymF s -> pure $ Direct $ symToBinding s
        VecF v -> Destructure <$> traverse (expectSym "assignment" . project) v
        _ -> failWith "Invalid type of assignment"


registerBnds ::
       (MonadReader e m, Field1' e DeclaredSymbols) => [Binding] -> m a -> m a
registerBnds bnds =
    local
        (_1 %~ DeclaredSymbols . HS.union (HS.fromList bnds) .
         unwrapDeclaredSymbols)


registerAssign ::
       (MonadReader e m, Field1' e DeclaredSymbols) => Assignment -> m a -> m a
registerAssign assign = registerBnds $ extractBindings assign


type JVMEnvExpr = Either (Unevaluated Object) (NLazy Object)
type JVMEnvExprs = Seq JVMEnvExpr
type AlgoMap = HM.HashMap ClST.Symbol Expression
type ToALangM
     = RWST (DeclaredSymbols, Registry) () (JVMEnvExprs, AlgoMap) (OhuaM Object)
...
mkEnvRef ::
       (MonadState s m, Field1' s JVMEnvExprs, ToEnvExpr e, MonadError Error m)
    => e
    -> m HostExpr
mkEnvRef thing = do
    i <- S.length <$> use _1
...
getEnvExpr :: (MonadError Error m, MonadState s m, Field1' s JVMEnvExprs)
           => HostExpr -> m JVMEnvExpr
getEnvExpr he = preuse (_1 . ix index) >>= maybe (throwError msg) pure
  where
    index = Types.unwrap he
    msg = "Invariant broken, no env expression with index " <> Str.showS index

-- Discuss algos with no inputs with sebastian!
toALang :: Registry -> AnnST Meta -> OhuaM Object (Expression, JVMEnvExprs)
toALang reg st = do
    (a, s, ()) <-
        runRWST
            (histo toAlangWorker st)
            (noDeclaredSymbols, reg)
            (mempty, mempty)
    a' <- assignIds a
    pure (a', s ^. _1)
...
logEnvValue he = do
    ...
    logDebugN $
        "Env expr [" <> showT he <> "]: " <> showT exprObj <> " : " <>
        showT (Clojure.type_ exprObj)
...
toAlangWorker ::
       Base (AnnST Meta) (Cofree (Base (AnnST Meta)) (ToALangM Expression))
    -> ToALangM Expression
        ...
        SymF s -> do
            ...
            if isLocal
                then Var . Local <$> expectUnqual s
                    ...
                    case (msf, malgo) of
                        (Just _, Just _) ->
                            failWith $ "ambiguous reference " <> Str.showS s
                        (Just sf, Nothing) -> pure $ Var $ Sf sf Nothing
                        (Nothing, Just algo) -> integrateAlgo s algo
                        (Nothing, Nothing) -> mkEnvExpr asST
        VecF v -> mkEnvExpr asST
        FormF [] -> failWith "Empty form"
        FormF (head:rest) ->
            let values = map extract rest
                trees = map Cf.unwrap rest
                annHead@(Annotated headAnn headVal) =
                    getCompose $ Cf.unwrap head
             in case headVal of
                    SymF sym
                        | isLetSym sym -> do
                            when (sym == letStarSym) $
                                logWarnN
                                    "let should not be expanded to `let*`.\
                                    \ This may cause compile failures. \
                                    \When macroexpanding use `ohua.util/macroexpand-all` instead."
                            case trees of
                                AnnVecF _ v:_ -> do
                                    let assigns =
                                            map (recast toST) $ filteri odd v
                                    assignedValues <-
                                        traverse extract $ filteri even v
                                    vs <- sequence $ tail values
                                    handleLet (handleStatements vs) $
                                        zip assigns assignedValues
                                _ -> failWith "Expected binding vector"
              -- NOTE: This assumes a form of `(algo [] ...)` (or `(fn
              -- [] ...)`) it currently does not handle things like
              -- `(fn ([] ...))` perhaps we should ...
                        | isAlgoSym sym -> do
                            when (sym == fnSym || sym == fnStarSym) $
                                logWarnN
                                    "DEPRECATED: The use of `fn` is deprecated,\
                                    \use `algo` instead."
                            when (sym == fnStarSym) $
                                logWarnN
                                    "fn should not be expanded to fn*. \
                                    \This may cause compile failures. \
                                    \When macroexpanding use \
                                    \`ohua.util/macroexpand-all` instead."
                            case trees of
                                AnnVecF _ v:_ -> do
                                    vs <- sequence $ tail values
                                    assigns <-
                                        traverse (handleAssign . recast toST) v
                                    ($) <$> mkLams assigns <*>
                                        registerBnds
                                            (assigns >>= extractBindings)
                                            (handleStatements vs)
                                _ ->
                                    failWith
                                        "Exprected binding vector in algo form"
                        | isIfSym sym -> do
                            (c, t, e) <-
                                case values
                    -- for support of no-else-statement
                      ...
                      -- pure (condition, then_, Nothing)
                                     ->
                                        failWith $
                                        "NOT SUPPORTED: If with only two arguments is not supported \
                                        \yet. See Issue #14."
                                    [condition, then_, else_] ->
                                        (,,) <$> condition <*> then_ <*> else_
                                    _ ->
                                        failWith $
                                        "Wrong number of arguments for `if`. Expected 3, got " <>
                                        Str.showS (length rest)
                            pure $
                                Var (Sf ifFunc Nothing) `Apply` c `Apply`
                                Lambda "_" t `Apply`
                                Lambda "_" e
                    _ -> do
                        (fn:args) <- sequence values
             -- I insert `unit` here as argument to functions with no
             -- arguments.  This is removed again after lowering to
             -- DFLang with `Ohua.Compat.JVM.cleanUnits`.  Be aware
             -- that the `unit` value itself is a hack and will break
             -- resolving env args when not properly removed!  Make
             -- sure to pay special attention to the unit value and
             -- its application again when coercing env args!
                        logDebugN $ "Meta info: " <> showT headAnn
                        case fn of
                            Var (Env e) -> logEnvValue e
                            _ -> pure ()
                        restWInit <-
                            case getCljSfnInitExpr annHead of
                                Nothing -> pure args
                                Just o -> do
                        ...
                        pure $
                            foldl'
                                (\e v -> e `Apply` v)
                                fn
                                (insertUnitExpr restWInit)
                        where insertUnitExpr [] = [unitExpr]
                              insertUnitExpr xs = xs
  where
    asST = embed $ fmap (recast id) stWithAnn :: AnnST Meta
    recast ::
           (Recursive t1, Corecursive a1, Base t1 ~ CofreeF f1 a2)
        => (f1 a1 -> Base a1 a1)
        -> t1
        -> a1
...
handleLet ::
       ToALangM Expression -> [(ClST.ST, Expression)] -> ToALangM Expression
...
shiftEnvExprs offset =
    mapRefs $ \case
        Env he -> Env $ makeThrow $ Types.unwrap he + offset
        a -> a


bndToFnName :: MonadOhua env m => Binding -> m QualifiedBinding
bndToFnName bnd =
    symbolFromString (Str.toString (Types.unwrap bnd)) >>= \case
        Unqual _ ->
            failWith $
            "Could not convert " <> Str.showS bnd <> " to function name"
        Qual fnName -> return fnName


symToBinding :: ClST.Symbol -> Binding
symToBinding = makeThrow . \case
...
definedBindings = cata $ \(Compose (Annotated _ val)) ->
  case val of
    SymF s -> HS.singleton $ symToBinding s
    e      -> F.fold e
